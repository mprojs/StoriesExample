...
import { IWrapHTTPConfig } from '@common/generated-api/base-service';

export const BusyServiceDisablingMarker = 'DisableBusyServiceForThisRequest';
...
abstract class Parameter {
  constructor(public name: string, public value: any, public options: ParameterOptions, defaultStyle: string,
  ...
  serializeValue(value: any, separator = ','): string {
    if (value === null || value === undefined) {
      ...
      return value.map(v => this.serializeValue(v).split(separator).join(encodeURIComponent(separator)))
        .join(separator);
...
export class RequestBuilder {
  ...
  build<T = any>(options?: {
  ...
  }, config?: IWrapHTTPConfig): HttpRequest<T> {
    ...
    if (config?.disableLoader) {
      httpHeaders = httpHeaders.append(BusyServiceDisablingMarker, 'true');
    }
